...
export async function applyBookOperation(access: BookAccess, operation: BookOperation): Promise<MutationResult> {
  ...
  switch (operation.type) {
    case "recipe.upsert": {
      ...
      for (const item of recipe.ingredients) statements.push(db.prepare("INSERT INTO ingredients (id, recipe_id, book_id, name, amount, amount_text, unit, note, sort_order) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)").bind(item.id, recipe.id, bookId, item.name.slice(0, 200), item.amount, item.amountText?.slice(0, 100) ?? null, item.unit.slice(0, 50), item.note?.slice(0, 300) ?? null, item.order));
      for (const step of recipe.steps) statements.push(db.prepare("INSERT INTO recipe_steps (id, recipe_id, book_id, text, sort_order) VALUES (?, ?, ?, ?, ?)").bind(step.id, recipe.id, bookId, step.text.slice(0, 6000), step.order));
