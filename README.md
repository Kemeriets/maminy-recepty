...
Если загрузка по кнопке не помогла: на компьютере, где фотографии отображаются, откройте **Ещё → Настройки → Резервная копия → Рецепты и фото · ZIP**. Убедитесь, что архив скачался успешно; перенесите файл на телефон обычным способом. На телефоне откройте **Ещё → Настройки → Резервная копия → Загрузить фото из ZIP**. Эта операция только добавляет снимки из архивной копии в кэш приложения; рецепты, избранное и содержимое Яндекс Диска не меняются. Для полного восстановления всей книги используется отдельная подтверждаемая команда в разделе импорта. Лимит кэша фото — 24 МБ: сначала сохраняются миниатюры; часть больших изображений по мере использования может удалиться из кэша. Не очищайте данные браузера и не удаляйте приложение до сохранения резервной копии.
