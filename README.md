...
Начиная с 1.3.2 нужные папки создаются до первой отправки рецептов или фотографий, в том числе при наличии старой локальной очереди. Одинаковые запросы создания папок объединяются, а конфликтующий файл не принимается за папку. В 1.3.3 ручная синхронизация не полагается на неточный флаг `navigator.onLine`, повторяет краткий сбой безопасных запросов чтения и показывает безопасную фазу ошибки.
