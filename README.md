...
Личная электронная семейная книга: сайт на русском языке, mobile-first PWA для Android и одновременно удобный настольный инструмент для переноса старых записей. Интерфейс работает без APK и Google Play, устанавливается на главный экран и сохраняет изменения локально даже без интернета.
...
- статический PWA размещается на **GitVerse Pages**;
- тот же PWA можно одновременно держать на **GitHub Pages** как независимый запасной адрес;
- рецепты и фотографии лежат в закрытой папке приложения на **Яндекс Диске**;
...
- фильтры по категории, автору, избранному и наличию фотографии;
...
- редактируемые категории и семейные авторы;
...
1. Войдите в Яндекс под аккаунтом владельца приложения и откройте [создание OAuth-приложения](https://oauth.yandex.ru/client/new/).
2. Название: `Мамины рецепты`.
3. Платформа: веб-сервис.
4. Redirect URI: полный адрес GitVerse Pages, обязательно с завершающим `/`, например `https://ВАШ_ЛОГИН.gitverse.site/maminy-recepty/`.
...
7. Вставьте его в `public/runtime-config.js`:
...
8. Сделайте новый commit и push. GitVerse автоматически обновит PWA.
...
3. На главном экране появится иконка «Мамины рецепты».
...
Категории и авторов можно добавлять в самом приложении. Значения из конфигурации нужны только для новой книги.
...
Выберите JSON и изображения одновременно либо положите их в один ZIP. Поддерживаются стратегии дубликатов: пропустить, заменить, добавить копию. Новые категории и авторы создаются автоматически.
...
Это не ошибка. Всё сохраняется локально. Для копии и других устройств откройте настройки и подключите семейное облако.
