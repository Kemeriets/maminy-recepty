...
Начиная с 1.3.2 нужные папки создаются до первой отправки рецептов или фотографий, в том числе при наличии старой локальной очереди. Одинаковые запросы создания папок объединяются, а конфликтующий файл не принимается за папку. В 1.3.3 ручная синхронизация перестала полагаться на неточный флаг `navigator.onLine`. В 1.3.4 приложение повторно получает временную ссылку Яндекс Диска после сбоя скачивания и загружает операции небольшими группами. Код `NET_DOWNLOAD_FAILED` означает, что API доступен, но отдельный сервер файлов не открылся; на телефоне проверьте VPN, Private DNS и блокировщик трафика. Wi‑Fi для синхронизации не требуется.
