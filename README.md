...
Маршрут уже развёрнут на `https://recepty-foto-kemeriets.netlify.app/media`, а сайт настроен на него. Владельцу проекта нужно открыть [настройки видимости этого проекта Netlify](https://app.netlify.com/projects/recepty-foto-kemeriets/configuration/access#visitor-access) и выбрать **Public** для production. Пока Netlify требует вход в его команду, браузер книги получает `401` и не может пересылать фото. Публичным становится только адрес функции: сам файл `/media/{id}` без действующего токена Яндекс Диска выдаёт `401`. Другие проекты аккаунта менять не нужно.
