...
Версия приложения: `1.3.11`.

Рабочий адрес: https://kemeriets.github.io/maminy-recepty/.

**Текущее состояние:** приложение опубликовано на GitHub Pages, служба передачи фото `recepty-foto-kemeriets` развёрнута и открыта для запросов приложения. Публичный `yandexClientId` и адрес службы заданы в `public/runtime-config.js`. Тексты и фотографии синхронизируются после входа в один Яндекс-аккаунт на обоих устройствах. Проверка службы отвечает `200 ready`, без Яндекс-авторизации фотографии недоступны (`401`). Перенос реального снимка проверьте в приложении после входа: доступ к личному Яндекс Диску для этого необходим. Не очищайте кэш ПК до проверки передачи фото. ClientID не является паролем или OAuth-токеном.
...
2. Ищите по названию или ингредиенту. Категории, **Все рецепты** и **Любимые** переключают разделы. В каталоге видны сразу все рецепты, фотографии подгружаются при прокрутке, а при возврате из рецепта сохраняется место в списке. **Фильтры** открывают компактное окно возле кнопки; изменения сразу видны в каталоге. Крестик в строке поиска очищает запрос.
3. В рецепте можно изменить порции с шагом 0,5, включить **Режим готовки**, отправить ингредиенты в **Покупки**, редактировать или поделиться полным текстом. Когда в старой записи выход не указан, 1 означает все ингредиенты исходного рецепта, 1,5 — полтора исходных рецепта. Это условный множитель, реальное число человек неизвестно. Пересчёт и отметки шагов не изменяют исходный рецепт.
...
5. После настройки службы фотографий новые фото передаются автоматически через закрытую папку Яндекс Диска, когда есть интернет, и подгружаются на втором устройстве при просмотре. Регулярно скачивайте **Настройки → Резервная копия → Рецепты и фото · ZIP** для независимого архива.
...
- после настройки и подключения рецепты и фотографии синхронизируются с закрытой папкой приложения на **Яндекс Диске**; двоичные фото могут проходить через отдельный прокси Netlify, который не хранит их;
...
flowchart TD
  ...
  SYNC --> EDGE["Netlify: передача фото без хранения"]
  EDGE --> YD
...
```text
Приложения/Рецепты/
  operation-metadata-v2/  сжатые части текстовых операций в приватных метаданных
  operations/             старые JSON-операции (читаются при миграции)
...
7. Для первого входа можно сразу вставить ClientID в **Настройки → Настроить синхронизацию → ClientID** и нажать **Сохранить и войти через Яндекс**. Идентификатор сохранится только в этом браузере. Для настройки всех устройств вставьте его в `public/runtime-config.js`:
   ...
   ```js
   globalThis.__MAMINY_RECIPES_CONFIG__ = {
...
### Автоматическая передача фото между телефоном и ПК

При прямой работе браузера с Яндекс Диском запросы на скачивание/отправку больших файлов уходят на отдельный адрес. На некоторых сетях он не открывается, даже если рецепты уже успешно синхронизированы. Небольшой **приватный прокси фотографий** берёт временную ссылку у Яндекс Диска и пересылает двоичный файл. Файлы постоянно хранятся только в закрытой папке Яндекса, Netlify их не сохраняет. Вход остаётся через тот же Яндекс-аккаунт на каждом устройстве. Передача в обе стороны автоматическая: фото сохраняется в рецепте, затем по появлении сети отправляется с исходного устройства и появляется на втором при открытии. Wi-Fi не требуется.

Маршрут уже развёрнут на `https://recepty-foto-kemeriets.netlify.app/media`, настроен в приложении и открыт для запросов. Отдельно заходить в Netlify для ежедневной работы не нужно. Публичным стал только адрес функции: сам файл `/media/{id}` без действующего токена Яндекс Диска выдаёт `401`.

Затем проверить ответ `200 ready` командой `curl -i -H 'Origin: https://kemeriets.github.io' https://recepty-foto-kemeriets.netlify.app/media/health`, открыть на ПК **Настройки** и убедиться, что появилась надпись «Служба передачи фото доступна». На телефоне обновить PWA и открыть ранее недоступное фото, затем добавить другое фото с телефона, дождаться синхронизации и открыть его на ПК. До проверки не удалять локальные данные с устройства, где фото уже открывается.

Если репозиторий перенесут на другой адрес, новый Netlify-сайт можно собрать по `netlify.toml`: `publish = "relay-site"`, функция обслуживает только `/media/*`. Адрес нового сайта записать в `public/runtime-config.js` как `mediaProxyUrl: "https://НОВЫЙ-САЙТ.netlify.app/media"` и опубликовать новую версию. Netlify не получает семейные рецепты из Git. **ClientID, Яндекс-аккаунт и основной адрес GitHub Pages не менять.**

Функция принимает только изображения в папке `app:/images`, проверяет Яндекс OAuth для каждого запроса, не публикует файлы по открытым ссылкам и не записывает токены в настройки Netlify. Она пропускает только запросы с сайта книги и ставит `Cache-Control: no-store`; исходные и просмотренные фото кэшируются **только браузером приложения**. Для полноценного backup по-прежнему нужен ZIP. Настройка URL прокси требуется только владельцу сайта, маме ничего настраивать не нужно.

По [текущим условиям Netlify](https://www.netlify.com/pricing/) Free включает функции и 300 credits в месяц с жёстким лимитом: после исчерпания лимита функция будет приостановлена до следующего месяца. Netlify не обещает вечную бесплатность. Для небольшой личной книги объём данных обычно гораздо ниже лимита; следите за использованием в кабинете. Если сервис недоступен, рецепты и уже сохранённые фото остаются на устройствах, новые файлы ждут повторной синхронизации.
...
Сохранение сначала фиксирует рецепт и очередь отправки одной транзакцией IndexedDB. Фотографии сжимаются до 1600 px (архивные страницы до 2400 px) и получают миниатюру. Текстовые операции сжимаются и передаются через метаданные основного API Яндекс Диска; отдельный файловый сервер для чтения рецептов больше не нужен. Фотографии остаются отдельными файлами в приватной папке. Новое устройство получает тексты всех рецептов, но не весь Диск и не все полноразмерные фотографии. Фото подгружаются при просмотре; миниатюры — для видимых карточек. Фото несохранённых черновиков не отправляются.

Изменения отправляются при сохранении. Повторная отправка и получение изменений происходят при возвращении в приложение, восстановлении интернета и примерно раз в минуту, пока приложение открыто. Есть ручная кнопка **Синхронизировать**. Для синхронизации подходит мобильный интернет, Wi‑Fi не требуется. Это не push-сервис: браузер может останавливать фоновую вкладку; перед выходом проверьте очередь. При сетевой ошибке книга остаётся локально, а индикатор не сообщает ложный успех. Одновременно редактировать один рецепт с двух устройств не рекомендуется: применяется последняя по времени операция.
...
Приложение проверяет обновление при запуске, возвращении на экран и восстановлении интернета. Новая версия активируется автоматически; при открытой форме рецепта перезагрузка откладывается до следующего запуска, чтобы не потерять ввод. В настройках есть ручная кнопка `Проверить обновление`. Старые кэши с другой версией удаляются.
...
export const APP_CONFIG = {
  ...
  dedicationText: "...",
  showGiftIntro: false,
  version: "1.3.8", // старое поле; номер нового релиза: config/version.ts
...
Service worker обслуживает приватные изображения через локальный URL и кэш IndexedDB. Если настроен `mediaProxyUrl`, он скачивает изображение через проверяющую OAuth функцию Netlify. Иначе запрашивает временную ссылку Яндекс Диска напрямую, что может не работать в некоторых сетях. Предел локального кэша просмотренных изображений — 24 МБ; неотправленные снимки сохраняются отдельно до успешной передачи.
...
Тесты покрывают CRUD, поиск, порции, импорт, экспорт, восстановление, IndexedDB, OAuth, повторную синхронизацию, выбор случайного блюда, очистку покупок и кэш фото. E2E smoke также проверяет полноэкранную готовку, компактные фильтры и длинные заметки. Для ручной визуальной проверки на компьютере откройте `layout-check.html` рядом с опубликованным приложением: он показывает реальные iframe-окна шириной 320 и 390 px без изменения данных или настройки сервера. Это вспомогательный инструмент разработчика, не часть интерфейса для мамы.

## Обновление приложения

1. Укажите новый номер в `config/version.ts`.
...
### Вход выполнен, но синхронизация завершилась ошибкой

Версия 1.3.6 больше не скачивает текст рецептов с отдельного файлового сервера Яндекса: сжатые операции читаются через основной API. Старые записи автоматически переносятся устройством, где они были созданы. После обновления сначала один раз нажмите **Синхронизировать** на исходном телефоне, затем — на компьютере. VPN и Wi‑Fi для этого не нужны; подходит мобильный интернет. Старые файлы не удаляются.

### Если на другом устройстве не видны фотографии

Синхронизация текстов и загрузка фотографий — разные запросы: для файлов Яндекс Диск выдаёт краткосрочную ссылку на отдельный файловый сервер. Статус «Рецепты синхронизированы» подтверждает текстовые записи, но не гарантирует доступность каждого фото. После обновления до 1.3.10 откройте приложение сначала на устройстве с неотправленным снимком, нажмите **Синхронизировать** и дождитесь завершения. Затем откройте рецепт на втором устройстве. Служба передачи поддерживает нумерованные адреса загрузки Яндекс Диска. Проверьте в настройках доступность службы передачи фото. ZIP ниже нужен только как аварийная мера, а не для ежедневного использования.

Для аварийного переноса ранее доступных только на одном устройстве фото откройте **Настройки → Резервная копия → Скачать фото для телефона**, перенесите ZIP и загрузите его на втором устройстве. Это локальный перенос, не замена автоматической передаче или полной резервной копии. Не очищайте данные браузера и не удаляйте приложение до сохранения полной резервной копии с фото.

В **Настройках** показаны причина и безопасный код ошибки. `HTTP 401` означает повторный вход, `HTTP 403` — проверку права `cloud_api:disk.app_folder` у созданного приложения и повторную авторизацию. Не добавляйте доступ ко всему Диску. `HTTP 507` означает нехватку места на Диске, `HTTP 429` — паузу перед повтором. Сетевой сбой и тайм-аут показываются отдельно. Произвольные сообщения API и URL с токенами не выводятся. Вход в аккаунт сам по себе ещё не означает успешную синхронизацию.

При ошибке очередь и неотправленные фото остаются локально. Не очищайте данные сайта и не создавайте другое OAuth-приложение: у него будет отдельная папка. Обновите PWA, повторите синхронизацию и при повторном сбое отправьте только безопасный код ошибки из настроек. Пароль, Client Secret и access token не отправляйте.
