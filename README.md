...
Версия приложения: `1.3.9`.

Рабочий адрес: https://kemeriets.github.io/maminy-recepty/.

**Текущее состояние:** приложение опубликовано на GitHub Pages. Публичный `yandexClientId` и адрес уже развёрнутой службы передачи фото заданы в `public/runtime-config.js`. Тексты синхронизируются после входа в одинаковый Яндекс-аккаунт на каждом устройстве. Для фотографий остаётся включить видимость **Public** только у проекта `recepty-foto-kemeriets` в Netlify: при обязательном входе в команду его Edge Function отвечает браузеру `401` (инструкция ниже). Не очищайте кэш ПК до проверки передачи фото. ClientID не является паролем или OAuth-токеном.
...
Маршрут уже развёрнут на `https://recepty-foto-kemeriets.netlify.app/media`, а сайт настроен на него. Владельцу проекта нужно открыть [проект `recepty-foto-kemeriets` в Netlify](https://app.netlify.com/projects/recepty-foto-kemeriets), выбрать **Make public** для production или перейти в **Project configuration → General → Visitor access → Project visibility** и выбрать **Public** для production. Пока Netlify требует вход в его команду, браузер книги получает `401` и не может пересылать фото. Публичным становится только адрес функции: сам файл `/media/{id}` без действующего токена Яндекс Диска выдаёт `401`. Другие проекты аккаунта менять не нужно.
...
Синхронизация текстов и загрузка фотографий — разные запросы: для файлов Яндекс Диск выдаёт краткосрочную ссылку на отдельный файловый сервер. Статус «Рецепты синхронизированы» подтверждает текстовые записи, но не гарантирует доступность каждого фото. Сначала выполните настройку **«Автоматическая передача фото между телефоном и ПК»** выше; обновите PWA на обоих устройствах, войдите в одинаковый аккаунт Яндекса, затем откройте рецепт заново. Проверьте в настройках доступность службы передачи фото. ZIP ниже нужен только как аварийная мера, а не для ежедневного использования.
