...
**Текущее состояние:** сайт опубликован на GitHub Pages. В `public/runtime-config.js` настроен публичный `yandexClientId`. Для синхронизации откройте настройки книги и войдите через Яндекс; на всех устройствах используйте один аккаунт. До входа изменения сохраняются только локально. ClientID не является паролем или OAuth-токеном. Исходники и обновления сайта не содержат ваших добавленных рецептов.
...
Синхронизация текстов и загрузка фотографий — разные запросы: для файлов Яндекс Диск выдаёт краткосрочную ссылку на отдельный файловый сервер. Статус «Рецепты синхронизированы» подтверждает текстовые записи, но не гарантирует доступность каждого фото. Версия 1.3.7 повторяет загрузку фото по кнопке, показывает основное изображение, когда миниатюра отсутствует, и подставляет ранее сохранённую миниатюру, если большой файл недоступен.

Если загрузка по кнопке не помогла: на компьютере, где фотографии отображаются, откройте **Ещё → Настройки → Резервная копия → Скачать фото для телефона**. Версия 1.3.8 создаёт отдельный ZIP с доступными на этом компьютере фотографиями и сообщает, сколько фото удалось включить, а сколько пропущено. Перенесите ZIP на телефон и выберите **Ещё → Настройки → Резервная копия → Загрузить фото из ZIP**. Это перенос фотографий, а не резервная копия рецептов: тексты, избранное и Яндекс Диск при загрузке не меняются. Существующий архив **Рецепты и фото · ZIP** по-прежнему остаётся полной резервной копией и выдаёт ошибку, если для неё не хватает изображения. Лимит кэша фото на телефоне — 24 МБ: сначала сохраняются миниатюры; часть больших изображений со временем может удалиться из кэша. Не очищайте данные браузера и не удаляйте приложение до сохранения резервной копии.
