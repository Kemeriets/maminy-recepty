...
export function describeSyncError(value: unknown): SyncIssue {
  ...
  if (networkError) {
    ...
    if (networkError.stage === "download") {
      return {
        ...
        message: "Не удалось перенести старую запись книги.",
        help: "Откройте обновлённое приложение на устройстве, где этот рецепт создавался, и один раз нажмите «Синхронизировать». Затем повторите синхронизацию здесь. Wi‑Fi не требуется, VPN тоже не нужен; изменения остаются на устройстве.",
        diagnostic,
      };
    }
  ...
  if (error instanceof YandexDiskError) {
    ...
    if (error.code === "PhotoRelayLinkError") return { kind: "service", message: "Служба фотографий не приняла ссылку Яндекса.", help: "Снимок остался на этом устройстве. Сообщите владельцу книги код ошибки ниже, пароль и токен не нужны.", diagnostic };
    if (error.code === "PhotoRelayTransferError" || error.code === "PhotoRelayServerError") return { kind: "service", message: "Не удалось передать фотографию в облако.", help: "Снимок остался на этом устройстве. Повторите попытку; если ошибка остаётся, сообщите владельцу книги код ниже.", diagnostic };
